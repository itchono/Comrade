...
REDUCED_INSTRUCTION_SET = [AuxilliaryListener,
                           MessageHandler,  Databases,  Echo, Lists]
...
cogs = [Databases, AuxilliaryListener, MessageHandler, General, Vault, Echo,
        ...
        BPC, RandomEvents, Lists, Polls, SelfPing, TexRender, Shoujo, Graphing, ActivityTracker
    ...
    if first_start:
        ...
        for name in ["Databases", "Users", "Vault", "Polymorph", "Emotes"]:  # load cogs in order
            try:
                ...
                print(f"WARN: Cog not loaded {name}")

        print("Startup completed in {:.2f} seconds.\nCurrent Local Time: {}".format(
            time.perf_counter() - start_time, localTime().strftime("%I:%M:%S %p %Z")))

        for server in client.guilds:
            await log(server, "Comrade {} is online.\nLogged in from {}.\nStartup done in {:.2f} seconds".format(VERSION, getHost(), time.perf_counter() - start_time))

        if NOTIFY_OWNER_ON_STARTUP:
            embed = discord.Embed(title="Comrade is Online", description="Version {}\nLogged in from {}.\nStartup done in {:.2f} seconds".format(
                VERSION, getHost(), time.perf_counter() - start_time))
